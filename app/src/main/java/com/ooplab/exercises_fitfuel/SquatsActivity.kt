            ...
            .setResultListener { result, inputImage ->
                ...
                if (allLandmarks.isNotEmpty() && allLandmarks[0].isNotEmpty()) {
                    ...
                    runOnUiThread {
                        ...
                        // Define variables outside the main block
// Main code
// Define points for left and right hips, knees, and ankles
                        // Define variables outside the main block
                                 // To count completed reps
                        var count = 0  // Keeps track of the number of reps
                        var stage = "Up"  // Track squat stage: "Up" or "Down"
                        var lastStage = "Up"  // Track previous stage to avoid double counting
                        ...
                        if (leftHip != null && leftKnee != null && leftAnkle != null &&
                            ...
                            // Define squat position condition (angles below 90 degrees means "Down")
                            val isInSquatPosition = (angleLeftKnee < 90.0) || (angleRightKnee < 90.0) ||
                                    (angleLeftHip < 90.0) || (angleRightHip < 90.0)

                            // Update stage and rep count
                            if (isInSquatPosition) {
                                if (stage == "Up" && lastStage == "Up") {
                                    // Transition from "Up" to "Down" when user goes into a squat position
                                    stage = "Down"
                                }
                            } else {
                                if (stage == "Down" && lastStage == "Down") {
                            ...
                            runOnUiThread {
                                ...
                                angle1TextView.text = "Angle Left Knee: $angleLeftKnee, Left Hip: $angleLeftHip"
                                angle2TextView.text = "Angle Right Knee: $angleRightKnee, Right Hip: $angleRightHip"
                            }
                        }
