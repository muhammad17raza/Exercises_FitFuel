...
// Check if all necessary landmarks are detected
                        if (leftShoulder != null && rightShoulder != null && leftHip != null && rightHip != null &&
                            ...
                            val averageTorsoToLegsAngle = (angleTorsoToLegsLeft + angleTorsoToLegsRight) / 2

                            // Define conditions for "up" and "down" positions
                            val isDownPosition = averageTorsoToLegsAngle >= 150 && averageTorsoToLegsAngle <= 180
                            val isUpPosition = averageTorsoToLegsAngle >= 30 && averageTorsoToLegsAngle <= 60

                            // Track sit-up movement
                            if (isDownPosition || !isSitUpPosition) {
                                // Starting the sit-up from the "down" position
                                stage = "Down"
                                isSitUpPosition = true

                            } else if (isUpPosition || isSitUpPosition) {
                                // Transitioning to the "up" position, count a rep
                                stage = "Up"
                                count++ // Increment sit-up count
                                isSitUpPosition = false
                            }
